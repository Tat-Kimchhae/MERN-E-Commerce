...
const Product = () => {
    ...
    return productData ? (
        <div className='border-t-2 pt-10 transition-opacit ease-in duration-500 opacity-100'>
            {/* Product Data */}
            <div className='flex gap-12 sm:gap-12 flex-col sm:flex-row'>
                    ...
                    <div className="flex sm:flex-col overflow-x-auto sm:overflow-y-scroll justify-between sm:justify-normal">
                        {
                            productData.image.map((item, index) => (
                                <img onClick={() => setImage(item)} src={item} key={index} className='w-[24%] sm:w-full sm:mb-3 shrink-0'/>
                            ))
                        }
                    </div>
                ...
                <div className="flex-1">
                        ...
                        <div className="flex gap-2">
                            {
                                productData.sizes.map((item, index) => (
                                    <button onClick={() => setSize(item)} className={`cursor-pointer border border-gray-100 py-2 px-4 bg-gray-100 ${item === size ? 'border-orange-300' : ''}`} key={index}> {item} </button>
                                ))
                            }
                        </div>
                    </div>
                    ...
                    <hr className="mt-8 sm:w-4/5 border-t-gray-200"/>
            ...
            <div className="mt-20">
                <div className="flex">
                    ...
                    <b className="border border-gray-300 px-5 py-3 text-sm"> Review (69) </b>
                </div>
